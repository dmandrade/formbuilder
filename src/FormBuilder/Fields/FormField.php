...
use Dmandrade\FormBuilder\FormHelper;
use Dmandrade\FormBuilder\RulesParser;
use Dmandrade\FormBuilder\Traits\Bindable;

/**
 * Class FormField
 *
 * @package Dmandrade\FormBuilder\Fields
 */
abstract class FormField
{
    ...
    protected function localizedRender($showLabel, $showField, $showError)
    {
        ...
        foreach ($this->options['locales'] as $locale => $localeOptions) {
            ...
            if (!isset($localeOptions['label']))
                $localeOptions['label'] = ucfirst($this->name) . ' (' . $locale . ')';
    ...
    public function setType($type)
    {
        if ($this->formHelper->getFieldType($type)) {
            return $this->parent->modify($this->name, $type, [], true);
        }
